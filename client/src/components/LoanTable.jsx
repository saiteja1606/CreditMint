            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  {showBorrower ? (
                    <Link to={`/borrowers/${loan.borrowerId}`} className="block truncate text-lg font-bold tracking-tight text-slate-900 transition-colors hover:text-brand-600 dark:text-white dark:hover:text-brand-400">
                      {loan.borrower?.name}
                    </Link>
                  ) : (
                  ...
                  <div className="mt-1.5 flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider">
                    <StatusBadge status={loan.status} />
                    <span className="text-slate-300 dark:text-slate-600">•</span>
                    <span className="text-slate-400">Due {formatDate(loan.dueDate)}</span>
                  </div>
                </div>
                ...
                <div className="relative">
                  <button 
                    ...
                    className={`p-2.5 rounded-2xl transition-all ${isMenuOpen ? 'bg-slate-100 dark:bg-slate-700 text-brand-600' : 'bg-slate-50 dark:bg-slate-900/50 text-slate-400 hover:text-slate-600'}`}
                  >
                  ...
                  <AnimatePresence>
                    {isMenuOpen && (
                      <motion.div
                        ...
                        className={`absolute right-0 z-50 w-52 rounded-2xl bg-white p-2 shadow-2xl ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-700 ${isLastItem ? 'bottom-12' : 'top-12'}`}
                      >
                        <Link 
                          to={`/loans/${loan.id}`}
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                ...
                <div className="rounded-2xl border border-slate-100 bg-slate-50/50 p-3.5 dark:border-slate-700/30 dark:bg-slate-700/20">
                  ...
                  <p className={`mt-1 text-base font-bold ${remaining > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
              ...
              <div className="mt-4">
                ...
                <div className="relative h-2.5 rounded-full bg-slate-100 dark:bg-slate-700/50">
                  <motion.div 
                    initial={{ width: 0 }}
                    animate={{ width: `${pct}%` }}
                    transition={{ duration: 1, ease: 'easeOut' }}
                    className="absolute inset-y-0 left-0 rounded-full bg-gradient-to-r from-brand-500 to-indigo-500 shadow-[0_0_12px_-2px_rgba(99,102,241,0.5)]" 
                  />
                </div>
                {loan.overdueInterest > 0 && (
                  <div className="mt-3 flex items-center gap-2 rounded-xl bg-amber-50 px-3 py-2 dark:bg-amber-900/10 border border-amber-100/50 dark:border-amber-900/20">
