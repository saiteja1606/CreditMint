...
import { calculateLoanPreview } from '../utils/finance'
...
export default function LoanModal({ isOpen, onClose, loan, borrowers, onSave }) {
  ...
  useEffect(() => {
    ...
    api.get('/wallet/summary')
      .then(res => { if (active) setWalletBalance(res.data.data.summary.walletBalance) })
      .catch(() => {})
    return () => { active = false }
  }, [isOpen, isEdit])
  ...
  const hasInsufficientBalance = walletBalance !== null && amountValue > walletBalance && !isEdit
  const preview = useMemo(() => calculateLoanPreview(amount, interestRate, interestType, startDate, dueDate), [amount, interestRate, interestType, startDate, dueDate])
    ...
    <Modal isOpen={isOpen} onClose={handleClose} title={isEdit ? 'Edit Loan' : 'New Loan'} size="lg">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          ...
          <select {...register('borrowerId')} className="input-base" disabled={isSubmitting}>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            <div className="flex items-center justify-between">
              <label className="label-base">Principal Amount (INR) *</label>
            ...
            <input {...register('amount')} type="number" step="0.01" min="1" placeholder="50000" disabled={isSubmitting} className={`input-base ${hasInsufficientBalance ? 'border-red-300 ring-red-500/10' : ''}`} />
          ...
          <div>
            ...
            <input {...register('interestRate')} type="number" step="0.01" min="0" placeholder="12" className="input-base" disabled={isSubmitting} />
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            {[{ val: 'SIMPLE', label: 'Simple (Flat %)' }, { val: 'MONTHLY', label: 'Monthly' }].map(opt => (
              <label key={opt.val} className="flex min-h-[48px] items-center gap-3 rounded-2xl border border-slate-200 px-4 py-3 dark:border-slate-700 cursor-pointer">
                <input {...register('interestType')} type="radio" value={opt.val} className="h-4 w-4 accent-brand-500" disabled={isSubmitting} />
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            ...
            <input {...register('startDate')} type="date" className="input-base" disabled={isSubmitting} />
            {errors.startDate && <p className="text-xs text-red-500 mt-1">{errors.startDate.message}</p>}
          </div>
          <div>
            <label className="label-base">Due Date *</label>
            <input {...register('dueDate')} type="date" className="input-base" disabled={isSubmitting} />
            {errors.dueDate && <p className="text-xs text-red-500 mt-1">{errors.dueDate.message}</p>}
          </div>
        </div>

        {preview && (
          <div className="grid grid-cols-1 gap-3 rounded-xl border border-brand-200 bg-brand-50 p-3 text-sm dark:border-brand-800 dark:bg-brand-900/20 sm:grid-cols-3">
            <div>
              <span className="text-brand-600 dark:text-brand-400">Monthly Interest: </span>
              <strong className="text-brand-700 dark:text-brand-300">{formatCurrency(preview.monthlyInterest)}</strong>
            </div>
            <div>
              <span className="text-brand-600 dark:text-brand-400">Profit: </span>
              <strong className="text-brand-700 dark:text-brand-300">{formatCurrency(preview.profit)}</strong>
            </div>
            <div>
        ...
        <div>
          ...
          <textarea {...register('notes')} rows={2} placeholder="Any additional notes..." className="input-base resize-none" disabled={isSubmitting} />
        </div>

        <label className="flex items-center gap-3 cursor-pointer">
          <input {...register('reminderEnabled')} type="checkbox" className="w-4 h-4 accent-brand-500" disabled={isSubmitting} />
